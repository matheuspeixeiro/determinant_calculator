/***	CALCULADORA DE DETERMINANTE DE MATRIZ QUADRADA 3X3
	...
	Nome: Matheus Simões Peixeiro		Código: 835733
   Curso: Engenharia da Computação		 Etapa: 1ª 	  ***/
...
main(){
	...
	for(i=0; i<3; i++){
		printf("| ");
		/* if-else para melhorar visualização } se positivo => 1 espaço
			para alinhar com o numero negativo */
		if(m[i][j] > 0) 
			printf(" %d\t", m[i][j]);
		else
			printf("%d\t", m[i][j]);
